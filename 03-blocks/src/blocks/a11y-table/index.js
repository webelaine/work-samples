...
registerBlockType("stmu/a11y-table", {
	...
	edit: props => {
		...
		var buttonsToEnable;
		// Caption
		var tableCaption, captionClass = 'is-hidden';
		if(showTable) {
			captionClass = '';
		}
		if(useCaption) {
		...
		var ariaLabel, rowCounter = 1;
		// Table Head
		var tableHead, headClass = 'is-hidden';
		if(showTable) {
			headClass = '';
		}
		...
		// Table Body
		var tableBody, formClass = '';
		const tableBodyData = dataBody
			...
			let rowCells = rows.bodyCells.map(function(cell, colIndex) {
				...
				let cellOptions = {
					...
					onInput: (evt) => {
						...
						newBody[rowIndex].bodyCells[colIndex] = newTh;
						// Set the attribute
						props.setAttributes({
							dataBody: newBody
						});
		...
		return (
			<div>
				<BlockControls key='a11y-form-controls'>
					<Toolbar>
						<Tooltip text='Insert Column Before'>
							<button
						...
						<Tooltip text='Insert Column After'>
							<button
								className='components-icon-button'
						...
						<Tooltip text='Insert Row Before'>
							<button
								className='components-icon-button'
				...
				<InspectorControls>
					<PanelBody title='Table options'>
						<PanelRow>
							<label htmlFor='toggle-caption'>Include a Caption</label>
							<FormToggle
								id='toggle-caption'
								checked={ useCaption }
						...
						<PanelRow>
							<label htmlFor='toggle-col-headings'>Include Column Headings</label>
							<FormToggle
						...
						<PanelRow>
							<label htmlFor='toggle-row-headings'>Include Row Headings</label>
							<FormToggle
						...
						<PanelRow>
							<label htmlFor='toggle-footer'>Include a Footer</label>
							<FormToggle
								id='toggle-footer'
								checked={ useFooter }
				...
				<form className={ formClass }>
					...
					<div>
						<label for='useColHeadings'>Add Column Headings</label>
						<input
							type='checkbox'
							id='useColHeadings'
							checked={ useColHeadings }
					...
					<div>
						<label for='useRowHeadings'>Add Row Headings</label>
						<input
							type='checkbox'
							id='useRowHeadings'
							checked={ useRowHeadings }
					...
					<div>
						<label for='addFooter'>Add Footer</label>
						<input
							type='checkbox'
							id='footerCheck'
							checked={ useFooter }
	...
	//////////////////// SAVE ////////////////////
	save: props => {
		var { attributes: { dataBody, dataCaption, dataFooter, dataHead, showTable, useCaption, useColHeadings, useFooter, useRowHeadings }, className } = props;
		let numCols = parseInt(props.attributes.numCols, 10);
		let numRows = parseInt(props.attributes.numRows, 10);
		// Caption
		var tableCaption;
		...
		// Table Body
		var tableBody;
		const tableBodyData = dataBody.map(function(rows) {
